...
output_csv = "single_patient_no_bandPass_noTrend.csv"
visualize = False
...
for psg in psg_files:
    ...
    # remove this if doign only for single patient
    break
...
common_channels = channel_sets[0] & set(fallback_channels)
chosen_channel = None
...
for i, (signal_file, hypnogram_file) in enumerate(subject_pairs):
    ...
    if chosen_channel not in raw.ch_names:
        print(f"  Channel {chosen_channel} not found in {signal_file}, skipping.")
        continue
    ...
    epochs = mne.Epochs(raw, events, event_id=None, tmin=0.0, tmax=30.0, baseline=None, detrend=None, verbose=False)
